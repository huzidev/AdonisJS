...
export default function ShowBlogs(props: any): JSX.Element {
  ...
  cosnt; { isViewProfile; } useShowBlogsHook();
          ...
          props.allBlogs.map((blog: any, index: number) => {
            ...
            return (
              ...
              <div
                key={blog.id}
                title="Read More"
                className={`w-[32%] mt-8 ${index % 3 === 1 && "mx-4"} margin cursor-pointer`}
                    ...
                    <div className="flex justify-between flex-col">
                      ...
                      </h1>
                      {
                        ...
                        auth.state.user && auth.state.user.role === "user" && (
                          <div
                            title={(favBlogs.find(
                              (favoriteBlog: any) => favoriteBlog.id === blog.id
                            )
                              ? "Remove From"
                              : "Add to") + " favorite"}
                            className={`p-2 rounded-full transition-colors duration-300 ${favBlogs.find(
                              (favoriteBlog: any) => favoriteBlog.id === blog.id
                            )
                                ? "text-red-500"
                                : "text-gray-500"}`}
                            onClick={() => favBlogs.find(
                              (favoriteBlog: any) => favoriteBlog.id === blog.id
                            )
            ...
            <h1 className="text-lg mb-6 font-bold tracking-tight text-white">
              ...
              {user.state.getUser.data?.role === "user" && !isMe
                ...
                isUser && isMe
                  ...
                  `${isMe ? "You" : props.userDetails.username} haven't uploaded`}{" "}
              any blog{" "}
